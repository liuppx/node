...
Commands:
  ...
  ./cmd secrets init|set|remove|passwd|unlock|verify|migrate-config|migrate [args]
...
run_secrets() {
  ...
  case "$action" in
    ...
    migrate)
      ...
      ;;
    *)
      fail "usage: ./cmd secrets init|set|remove|passwd|unlock|verify|migrate-config|migrate [args]"
      ;;
  esac
}
