...
async function createIdentityFixture(walletIdentityId: string) {
  ...
  const unsigned = {
    version: 1, id: identity, walletIdentityId, createdAt: '2026-08-13T00:00:00.000Z', updatedAt: '2026-08-13T00:00:00.000Z', revision: 1,
  ...
  return { identity, document: { ...unsigned, proof: { type: 'YeyingIdentityDocumentProofV1', verificationMethod: `${identity}#${controllerId}`, proofValue: signature } } }
}

const primary = await createIdentityFixture('wid_1234567890123456789012')
...
describe('identity account link', () => {
  ...
  it('rejects linking the same active account to a different wallet identity', async () => {
    const wallet = Wallet.createRandom()
    const first = await issueAccountLinkChallenge({ identity, account: { chainKey: 'eip155:1', address: wallet.address } })
    await verifyAccountLink({ identityDocument: document, identity, account: first.account, nonce: first.nonce, issuedAt: first.issuedAt, expiresAt: first.expiresAt, accountSignature: await wallet.signMessage(first.message) })

    const second = await createIdentityFixture('wid_abcdefghijklmnopqrstuv')
    const secondChallenge = await issueAccountLinkChallenge({ identity: second.identity, account: { chainKey: 'eip155:1', address: wallet.address } })
    await expect(verifyAccountLink({ identityDocument: second.document, identity: second.identity, account: secondChallenge.account, nonce: secondChallenge.nonce, issuedAt: secondChallenge.issuedAt, expiresAt: secondChallenge.expiresAt, accountSignature: await wallet.signMessage(secondChallenge.message) })).rejects.toThrow('IDENTITY_ACCOUNT_ALREADY_LINKED')
  })
})
